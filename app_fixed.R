...
server <- function(input, output, session) {
  ...
  # Fonction pour créer un graphique de diagnostic pour un scénario donné
  create_scenario_plot <- function(data, horizon, title) {
    # Créer un graphique avec ggplot2
    p <- ggplot(data, aes(x = horizon, y = !!sym(col_name), color = scenario)) +
      geom_line() +
      geom_point() +
      labs(title = title, x = "Horizon", y = "Valeur") +
      theme_minimal() +
      theme(legend.position = "bottom")
    return(p)
  }
  
  # Fonction pour générer un graphique de diagnostic simplifié
  ...
  # Fonction pour obtenir la légende d'un graphique
  ...
  # Fonction pour charger et mettre en cache les données Excel
  load_excel_files <- reactive({
    # Vérifier le cache
    if (!is.null(session$userData$excel_data)) {
      print("Utilisation des données Excel en cache")
